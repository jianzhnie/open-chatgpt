...
def inference(model, tokenizer):
    ...
    count = 0
    for post, summarize in tqdm(zip(test_post_list, test_summ_list),
                                total=len(test_post_list)):
        encode_dict = tokenizer(post,
                                return_tensors='pt',
                                padding=False,
                                truncation=True)
        ...
        if count % 10 == 0:
            result = rouge.compute(predictions=pred_list,
                                   references=summarize_list)
            print(result)
        count += 1
    df = pd.DataFrame.from_dict({
        'pred': pred_list,
        'truth': summarize_list,
        'post': post_list
    })
