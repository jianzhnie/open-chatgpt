...
key_list = [
    key for key, _ in model.base_model.model.named_modules()
    if 'lora' not in key
]
for key in key_list:
    ...
    if isinstance(target, peft.tuners.lora.Linear):
        ...
        new_module = torch.nn.Linear(target.in_features,
                                     target.out_features,
                                     bias=bias)
        model.base_model._replace_module(parent, target_name, new_module,
                                         target)
