...
from .data_types import PPORLBatch, PPORLElement, RLElement
    ...
    def create_loader(self, batch_size: int, shuffle: bool) -> DataLoader:
        ...
        def collate_fn(elems: Iterable[PPORLElement]):
            return PPORLBatch(
                ...
                pad_sequence([elem.query_tensor.flip(0) for elem in elems],
                             padding_value=self.pad_token_id,
                             batch_first=True).flip(1),
                # Right pad the rest, to have a single horizontal query/response split
                pad_sequence([elem.response_tensor for elem in elems],
                             padding_value=self.pad_token_id,
                             batch_first=True),
                pad_sequence([elem.logprobs for elem in elems],
                             padding_value=0.0,
                             batch_first=True),
                pad_sequence([elem.values for elem in elems],
                             padding_value=0.0,
                             batch_first=True),
                pad_sequence([elem.rewards for elem in elems],
                             padding_value=0.0,
                             batch_first=True))
