...
class RankGenCollator:
    ...
    def __call__(self, batch: List[Dict[str, str]]) -> Dict[str, torch.Tensor]:
        ...
        tokenized_prefixes = self.tokenizer(prefixes,
                                            return_tensors='pt',
                                            padding=self.padding,
                                            max_length=self.max_length,
                                            truncation=True)
        tokenized_pos = self.tokenizer(better_answers,
                                       return_tensors='pt',
                                       padding=self.padding,
                                       max_length=self.max_length,
                                       truncation=True)
        tokenized_neg = self.tokenizer(worse_answers,
                                       return_tensors='pt',
                                       padding=self.padding,
                                       max_length=self.max_length,
                                       truncation=True)
...
class HFSummary(Dataset):
    """Human feedback data from OpenAI https://github.com/openai/summarize-
    from-feedback.
    ...
    def __init__(self,
                 split='train',
                 conf_threshold=-1,
                 max_comparison_per_sample=1) -> None:
        super().__init__()
        ...
        dataset = load_dataset('openai/summarize_from_feedback',
                               'comparisons')[major_split]
        for data in dataset:
            if ('extra' in data and 'confidence' in data['extra']
            ...
            summaries[context].append((data['summaries'][pos]['text'],
...
class HFDataset(Dataset):
    """This is a base huggingface dataset which written to support the simplest
    pos-neg pair format.
